...
func main() {
	...
	r.POST("/", func(c *gin.Context) {
		...
		owner, _ := jsonparser.GetString(buf.Bytes(), "repository", "owner", "username")

		if owner != os.Getenv("OWNER") {
			fmt.Printf("Owner not allowed. owner from webhook=%q,owner from environment variable=%q\n", owner, os.Getenv("OWNER"))
			return
		}
